...
while episode < 10:
    action = env.action_space.sample() # Simple action
...
df.to_parquet("./data/cartpole_baseline.parquet")

plt.figure(figsize=(10,6), dpi=300)
plt.plot(df["episode"], df["reward"], color='blue', marker='o', linestyle='dashed', linewidth=1, markersize=3)
plt.xlabel("Episode")
plt.ylabel("Reward")
plt.title("CartPole-v1 Baseline")
plt.grid()
plt.savefig("figs/cartpole_baseline.png", dpi=300)
